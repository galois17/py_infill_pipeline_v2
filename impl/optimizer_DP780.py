...
class OptimizerDP780(OptimizerCPM):
    ...
    def calc_error(self, exp_x, exp_fcn, fit_range, sim_modx, sim_mody):
        """ Calculate the error 
        ...
        truthy = np.array([exp_x, exp_fcn])
        truthy.shape
        truthy_new = np.reshape(truthy, (-1,2) )

        sim_new = np.array([sim_modx, sim_mody])
        sim_new = np.reshape(sim_new, (-1,2) )
        
        err = sm.similaritymeasures.pcm(truthy_new, sim_new)
        
        assert err >= 0, "Error should be >= 0"

        return err, len(exp_x)
